...
Upload a photo and the model says which of the six scenes it is:
buildings, forest, glacier, mountain, sea, street.

The model is the one trained in scene-classification.ipynb, it is only loaded
...
MODEL_PATH = os.path.join(BASE_DIR, "scene_model.keras")
...
st.set_page_config(page_title="Scene Classification", page_icon="🏞️")

st.title("🏞️ Scene Classification")
st.write(
    "Upload a landscape or travel photo and the model will sort it into one of "
    "six scene categories. Trained on the Intel Image Classification dataset "
...
if uploaded is None:
    ...
    st.write(" · ".join(f"{name}: {p:.1%}" for name, p in ranked))
